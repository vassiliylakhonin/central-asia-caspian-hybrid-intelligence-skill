...
**Last updated:** 2026-08-24 (the root skill now carries the complete runtime-neutral contract; Claude, Codex and OpenClaw files are additive overlays. The four existing `analyze` agent-eval delta cases remain compatibility evidence for the older strategic-intelligence runtime.)
...
| B2.4 Platform differentiation or consolidation across `runtimes/{codex,claude,openclaw}` | ✅ met | Common behavior is consolidated in the root `SKILL.md`. Claude adds `Claude Tool-Use Awareness`; Codex adds `Codex Agentic-Loop Awareness`; OpenClaw inherits the root behavior and adds only ClawHub packaging. The validator rejects duplicated common sections and non-allowlisted overlay sections. |
